...
pub static MSR_POLICY_TABLE: &[MsrEntry] = &[
    ...
    MsrEntry {
        first: 0x174,
        last: 0x176,
        read: MsrAction::AreaSwap,
        write: MsrAction::AreaSwap,
        name: "IA32_SYSENTER_CS/ESP/EIP",
    },
    MsrEntry {
        first: 0x179,
        last: 0x179,
    ...
    MsrEntry {
        ...
        read: MsrAction::Passthrough,
        write: MsrAction::Passthrough,
        name: "IA32_THERM/MISC/MCA zone",
    },
    MsrEntry {
        first: 0x1d9,
        last: 0x1d9,
        read: MsrAction::Emulate,
        write: MsrAction::Emulate,
        name: "IA32_DEBUGCTL",
    },
    MsrEntry {
        first: 0x200,
    ...
    MsrEntry {
        ...
        read: MsrAction::Passthrough,
        write: MsrAction::Passthrough,
        name: "IA32_PERF_GLOBAL_CTRL/OVF_CTRL",
    },
    MsrEntry {
        first: 0x400,
        last: 0x47f,
        read: MsrAction::Passthrough,
        write: MsrAction::Passthrough,
        name: "IA32_MCi_CTL/STATUS/ADDR/MISC",
    },
    MsrEntry {
        first: 0x480,
        last: 0x4bf,
    ...
    MsrEntry {
        ...
        read: MsrAction::AreaSwap,
        write: MsrAction::AreaSwap,
        name: "IA32_STAR/LSTAR/CSTAR/SFMASK",
    },
    MsrEntry {
        first: 0xc000_0100,
        last: 0xc000_0102,
        read: MsrAction::AreaSwap,
        write: MsrAction::AreaSwap,
        name: "IA32_FS_BASE/GS_BASE/KERNEL_GS_BASE",
    },
    MsrEntry {
        first: 0xc000_0103,
        last: 0xc000_0103,
...
mod tests {
    ...
    fn test_lookup_boundaries() {
        ...
        assert_eq!(
            policy_lookup(0x174).unwrap().name,
            "IA32_SYSENTER_CS/ESP/EIP"
        );
        assert_eq!(
            policy_lookup(0x176).unwrap().name,
            "IA32_SYSENTER_CS/ESP/EIP"
        );
        assert_eq!(
            policy_lookup(0xc000_0102).unwrap().name,
    ...
    fn test_action_semantics() {
        ...
        assert_eq!(
            policy_lookup(0xc000_0102).unwrap().write,
            MsrAction::AreaSwap
        ); // KERNEL_GS_BASE
        assert_eq!(
            policy_lookup(0xc001_0015).unwrap().write,
            MsrAction::Passthrough
        ); // AMD HWCR
        ...
        assert_eq!(
            policy_lookup(0xc000_0080).unwrap().read,
            MsrAction::AreaSwap
        );
        assert_eq!(
            policy_lookup(0xc000_0100).unwrap().write,
            MsrAction::AreaSwap
        );
        assert_eq!(
            policy_lookup(0xc000_0101).unwrap().write,
            MsrAction::AreaSwap
        );
    ...
    fn test_linux_hot_path_msrs_are_covered() {
        ...
        for msr in [
            ...
            assert!(
                policy_lookup(msr).is_some(),
                "MSR {:#x} missing from policy table",
                msr
            );
        }
    }
